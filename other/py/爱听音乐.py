...
class Spider(Spider):
    ...
    def _parse_list(self, items, tid=""):
        ...
        for li in items.items():
            ...
            # 歌手页：圆形，比例 0.8（缩小至原来的4/5）
            # 其他页：矩形，比例 2.66（高度减半）
            if "/singer/" in href:
                style = {"type": "oval", "ratio": 0.9}
            else:
                style = {"type": "rect", "ratio": 3.99}
    ...
    def _fetch_filters(self, url):
        doc, filters = self.getpq(url), []  # 注意：这里原代码有误，doc 应该来自 getpq，但下面用了 doc(s) 是OK的
        ...
        for attempt in range(max_retries):
            try:
                ...
                r = (self.session.post if method == "POST" else self.session.get)(
                    f"{self.host}{path}", params=params, data=data, headers=h,
                ...
                if "安全人机验证" in r.text:
                    if attempt == max_retries - 1:
                        return ""
                    self._auto_verify(f"{self.host}{path}")
                    continue
                if loc := r.headers.get("Location"):
                    return self._abs(loc.strip())
                try:
                    json_data = r.json()
                    return self._abs(json_data.get("url", "").replace(r"\/", "/")) or ""
                except:
                    return self._abs(r.text.strip()) if r.text.strip().startswith("http") else ""
            except:
                time.sleep(0.5)
                continue
        return ""
    ...
    def getpq(self, url):
        ...
        for attempt in range(max_retries):
            try:
                resp = self.session.get(self._abs(url), timeout=5)
                if "安全人机验证" in resp.text:
                    if attempt == max_retries - 1:
                        return pq("<html></html>")
                    self._auto_verify(self._abs(url))
                    continue
                return pq(resp.text)
            except:
                time.sleep(0.5)
